...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    ...
    const owner = normaliseQuery(req.query.owner);
    const signature = normaliseQuery(req.query.signature);

    try {
      if (scope === 'all') {
        if (!owner || !signature) {
          return res.status(401).json({ error: 'Owner address and signature required' });
        }
        const authorised = await verifyOwner(owner, signature);
        if (!authorised) {
          return res.status(401).json({ error: 'Invalid merchant signature' });
        }
